...
- 3 완료: 최근 이력 overview에 4레인 악보형 트랙 + 시간 버킷 기반 밀도 표현 + 결과 범례 반영
- 4 완료: `dialog` semantics, `aria-expanded`, 포커스 이동, 실시간 상태 요약(`aria-live`) 반영
- 5 완료: UI 회귀 보강 후 `npm run qa` 재통과
